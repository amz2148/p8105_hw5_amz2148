...
iter_results_df = 
  ...
  mutate(
    estimate_df = map(true_mean, iter_t_test)
  ) %>% 
  unnest(estimate_df) %>% 
  janitor::clean_names()
...
iter_results_plot = 
  iter_results_df %>% 
  group_by(true_mean) %>% 
  summarize(prop_reject = (sum(estimate <= 0.05) / n())) %>% 
  ggplot(aes(x = true_mean, y = prop_reject)) + 
  geom_point(alpha = .5, color = "black") +
  geom_smooth(se = FALSE, color = "#d98fcc") + 
  theme(plot.title = element_text(size = 20, face = "bold")) +
  ...
  labs(
      ...
      y = "Proportion of Rejected Nulls ('Power')")
...
-   Finally, we make 2 plots: one plot showing the average estimate of
    mu-hat on the y axis and the true value of mu on the x axis, and a
    second plot with the average estimate of mu-hat only in samples for
    which the null was rejected on the y axis and the true value of mu
    on the x axis.
...
<img src="p8105_hw5_amz2148_files/figure-gfm/graph2-2.png" width="100%" />
